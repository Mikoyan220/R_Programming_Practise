...
```{r}
bookings_df <- read_csv("C:/Users/user/Documents/Code/RStudio/dataset_hotel_bookings.csv")
```
...
```{r}
example_df <- bookings_df %>%
  mutate(total_pax = (adults + children + babies))
...
```{r}
# Grasp the total number of rows in the dataset
total_rows <- nrow(example_df)

# Convert the total_rows to an integer
total_rows <- as.integer(total_rows)

example_df <- bookings_df %>% 
  mutate(number_canceled = sum(is_canceled)) %>% 
    ## option 2: mean(lead_time)
  mutate(average_lead_time = sum(lead_time) / total_rows) 
